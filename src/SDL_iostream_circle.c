...
/* circle-coupe patch: also exclude FD backend on bare-metal (no POSIX FDs)
 * SDL_fdatasync is defined in this block and used by stdio_flush below,
 * so we stub it for Circle. */
#if !defined(SDL_PLATFORM_WINDOWS) && !defined(SDL_PLATFORM_CIRCLE)

// Functions to read/write file descriptors. Not used for windows/Circle.
...
#ifdef SDL_PLATFORM_CIRCLE
/* circle-coupe: stub SDL_fdatasync (no POSIX FDs on bare-metal) */
static int SDL_fdatasync(int fd) { (void)fd; return 0; }
#endif
